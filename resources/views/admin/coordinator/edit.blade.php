...
@section('content')
    <form class="form-horizontal" action="{{ route('admin.coordinator.update', ['id' => $coordinator->id]) }}" method="post">
        @method('PUT')
        @csrf
                    ...
                    <div class="col-sm-10">
                        <select class="form-control selection" id="inputTempOf" name="tempOf">
                            ...
                            @foreach((App\Models\Course::all()->find(old('course')) ?? $courses->first())->non_temp_coordinators as $coord)
                                ...
                                <option
                                    value="{{ $coord->id }}" {{ (old('tempOf') ?? $coordinator->temp_of ?? 0) == $coord->id ? 'selected=selected' : '' }}>
                                    {{ __($coord->user->name) }}
                                </option>
                        ...
                        <div class="form-group @if($errors->has('startDate')) has-error @endif">
                            ...
                            <div class="col-sm-8">
                                <input type="date" class="form-control" id="inputStartDate" name="startDate"
                                       value="{{ old('startDate') ?? $coordinator->start_date->format("Y-m-d") }}"/>
                            ...
                            <div class="col-sm-8">
                                <div class="input-group">
                                    ...
                                    <input type="date" class="form-control" id="inputEndDate" name="endDate"
                                           value="{{ old('endDate') ?? $coordinator->end_date->format("Y-m-d") }}">
                                </div>
